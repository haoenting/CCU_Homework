...
class DStar:
    ...
    def ComputePath(self):
        while True:
            s, v = self.TopKey() # 檢查當前估計值最小 s是點 v是值
            # 檢查是否回到先前路徑
    ...
    def h(self, s_start, current):
        heuristic_type = self.heuristic_type  # heuristic type

        if heuristic_type == "manhattan":
            return abs(current[0] - s_start[0]) + abs(current[1] - s_start[1])
        else:
            return math.hypot(current[0] - s_start[0], current[1] - s_start[1])
